...
`data/` and `.env` are ignored so worlds and secrets are not uploaded. Create an empty GitHub repository, then run:

```powershell
git init
git add docker-compose.yaml README.md .env.example .gitignore LICENSE data/.gitkeep
git commit -m "Initial Minecraft server template"
git branch -M main
git remote add origin https://github.com/YOUR-USERNAME/YOUR-REPOSITORY.git
git push -u origin main
```

The included MIT license lets others use and modify this template.
